...
6. **改 `State` 的字段时,三处必须一起动**。`Clone` 要加一行(漏了回滚时该字段不退回,
   下一轮重新产生的同一批事件被计第二次,日报数字凭空翻倍);`Load` 要加 nil 初始化
   (漏了新装用户一切正常、测试也全绿,只有存量用户升级后写入那个 map 的瞬间 panic,
   开发与 CI 都碰不到这条路径)。这两处各有一个反射测试守着,加字段就会红,不必手动登记。
   第三处没有东西能替你做:**`stateVersion` 该不该升**。升了,所有存量用户都得删文件
