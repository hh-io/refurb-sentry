...
>
> 它**默认开启**，因为这个工具的常态是配一条窄规则等上几个月：期间“手机安静”既可能是没货，也可能是进程挂了、或规则写错了永远不会命中，而三者需要的行动完全不同。不想要就把 `notify.daily_summary` 写成空串；配置文件是只读挂载时（compose 就是这么挂的）用 `REFURB_DAILY_SUMMARY=` 关闭。
>
