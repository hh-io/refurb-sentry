...
> [!WARNING]
> **这四家业务失败时也返回 HTTP 200**,把错误码放在响应体里,比如钉钉的
> `{"errcode":310000,"errmsg":"keywords not in content"}`、飞书的 `{"code":19024}`。
> 通用 webhook 只看 HTTP 状态码,因此这类失败会被当作推送成功,基线照常推进,
> **这批事件就永久丢失了**,日志里只会看到「推送成功」。
> 配好之后务必真发一条确认手机上收得到,别只看日志——尤其是选了自定义关键词的时候。
