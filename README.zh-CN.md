...
- 🗂️ **历史档案**：默认把每次上架、调价、下架追加记录到本地文件，几个月后用 `-history` 查「某个配置以前上架过几次、各卖多少钱、多久被买走」，详见[历史档案](#历史档案)。
...
- **不做价格走势图**：历史档案按「每次售卖」记录上下架时间与价格，供命令行查询，不做图表。
...
| `-history` | 查询历史档案后退出，查询参数见[历史档案](#历史档案) |
| `-version` | 打印当前版本号 |
...
- **开了开关却没有规则按内存过滤、且关闭了历史档案时，启动会告警**：这种情况补齐会被整个跳过
  （补了也改变不了任何结果），启动警告会说出来，不必自己猜为什么没变化。
- **整个分类没有内存维度时一件都不抓**：watch 这类分类会被整体跳过。
  这不只是省请求——实测手表详情页会让 28 件全部「解析出内存」，
  那其实是存储容量，不设这道闸就会污染 `tsMemorySize`。
- **历史档案开启（默认）时全查，关闭时按需查**：档案要能按内存查询，因此开启时
  缺内存且有容量锚点的商品都会补。关闭档案时只有「除内存外其余条件都还可能命中某条规则」
  的商品才会去看详情页——机型、芯片、容量、价格列表页都已给全，凭它们就能否决的商品，
  内存再合适也不会命中；没有任何规则按内存过滤时则完全不查。

代价是首轮启动会慢一些。实测 CN mac 开启档案时补了约 45 件（按默认 1~3 秒间隔约 2 分钟）；
关闭档案、只用本文档示例的 MacBook Pro 规则时，58 件缺内存商品里只有 19 件值得查（约 50 秒）。
结果按货号缓存，之后稳态每轮只为新上架的商品多发请求；缓存只在内存里，重启后首轮会重新补一遍。

---

## 历史档案

同一个配置会在翻新店里反复上架。几个月后准备入手时，想知道「它以前出现过几次、每次卖多少钱、
多久被买走」，状态库回答不了——它只保留当前在架的商品，价格被原地覆盖、下架即删除。
历史档案补上这一块：**默认开启**，每次上架、调价（涨价也记）、下架各追加一行到 `history.jsonl`。
...
（输出为示意。）查询参数都要配合 `-history` 使用，单独给出会报错；多个取值用逗号分隔，之间为 OR：

| 参数 | 说明 |
|---|---|
| `-rule` | 按配置里同名的规则过滤，可与下列参数叠加（AND） |
| `-region` / `-category` | 地区 / 分类 |
| `-model` | 机型（`refurbClearModel`），如 `macbookpro` |
| `-chip` | 芯片，如 `"M5 Pro,M5 Max"` |
| `-memory` / `-storage` | 内存 / 存储，如 `32gb,36gb`、`1tb` |
| `-color` | 颜色（`dimensionColor`），如 `silver` |
| `-nano` | `true` 只看纳米纹理玻璃，`false` 只看标准玻璃 |
| `-title` | 标题正则（跑在归一化后的标题上） |

- 匹配**复用规则引擎**，语义与 `rules` 完全一致：维度缺失判为不匹配，芯片与核心数从标题解析。
- 结果按配置分组，最近有动静的在前；同一配置每次上架占一行。`≤` 表示建档时它已经在架，
  真实上架时刻只知道不晚于这个时间，在架时长因此标为 `≥`；下架时间前的 `≤` 同理，见[记录格式与可靠性](#3-记录格式与可靠性)。
- 按内存查询时，缺内存维度的记录不会悄悄消失，末尾会提示「另有 N 次售卖缺少内存维度，无法判断是否匹配」。
- 只读本地文件，不联网，不需要配置推送渠道。
...
history:
  enabled: true              # 默认开启
  path: ""                   # 留空则与 state_path 同目录，默认即 data/history.jsonl
  categories: [ipad, watch]  # 只归档、不推送的额外分类
```

- **默认开启**：档案无法事后补录，等到想查时才打开，之前那几个月就已经丢了。
- **默认路径跟随 `state_path`**：Docker 的命名卷、systemd 的 `StateDirectory` 覆盖的都是状态所在的目录，无需额外挂载。
- **`categories` 只归档、不推送**：比如只想被通知 Mac，又想留下 iPad 的价格记录。这些分类照常抓取
  （每个分类每轮多一个请求，同样命中 CDN 缓存）、建立基线、写入档案，但事件在规则匹配**之前**就被挡掉——
  没写 `categories` 的规则、空规则集也推不出去，同样不计入日报。之后把它挪进顶层 `categories`
  也不会刷屏，基线早就建好了。
- **显示器在 `mac` 分类里**（`refurbClearModel: display`），不必另加。
- 开启档案时，`http.fill_missing_memory` 的补齐不再以规则为前提，见[上一节](#5-内存维度缺失与-fill_missing_memory)。

### 3. 记录格式与可靠性

每行是一个完整的 JSON 对象，`kind` 取 `baseline`（建档时已在架）、`listed`、`price`、`delisted`：
...
- **只在本轮基线确定提交时写入**：推送失败回滚的那一轮不写，下一轮重试成功时才写，不会重复记录；`-dry-run` 不写。
- 首次开启时，状态库里已在架的商品会记为 `baseline`，上架时间取状态库里的首次发现时间，不会缺记录。
- **每次启动后会与状态库对一次账**：档案里仍在架、状态库里却已没有的商品（档案关闭期间、或状态文件删掉重建之前就下架了），补记一条下架，时刻标为近似，查询里显示为 `≤`，在架时长相应标为上限或「未知」。否则它们会永远停在「在售」。
- 追加写入并 fsync；断电留下的残行在读取时跳过并告警，不影响其余记录。
- 写入失败（如磁盘满）只记 ERROR 日志，不影响推送与基线。
- `spec` 是给外部工具用的便利字段，`-history` 总是从 `title` 重新解析——解析器日后修正了，旧记录跟着受益。
- **纳米纹理只存在于标题里**，按各站实测写法识别（`Nano-texture`、`Nanotextur`、`nanotextuur`、
  `nano-texturé`、`nanotexture`、`nanotexturizada`、`纳米纹理`、`納米紋理`、`奈米紋理`）；
  KR 站实测时没有带纳米纹理的在售商品，写法未经核实。
...
程序本身不写任何日志文件，日志输出至 stderr（控制台通知、维度列表与 `-history` 查询结果输出至 stdout），交给各自的守护方式收集。稳态下每个轮询周期
...
[历史档案](#历史档案)则只增不减，也不做轮转：每行约 600 字节，CN 的 mac + ipad + watch
建档约 400 行、230KB，之后随上架频率增长。不想要它就设 `history.enabled: false`。
...
状态文件默认保存于 `data/state.json`，记录在售商品的货号、标题、当前价格、规格维度及首见/末见时间。历史档案 `history.jsonl` 与它同目录，写入时机见[历史档案](#3-记录格式与可靠性)。采用“临时文件写入 + 原子 Rename”策略，任何异常断电或强行 Kill 均不会损坏状态库。想从零重建基线，直接删掉这个文件即可，下次启动会静默重新建立。
